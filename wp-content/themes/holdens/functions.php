...
// Custom Walker Class - Add this to your functions.php
class Custom_Nav_Walker extends Walker_Nav_Menu {
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        // Add ACF image if it exists (for sub-menu items)
        if ($depth > 0) { // Only for sub-menu items
            $sub_menu_image = get_field('sub_menu_image', $item);
            if ($sub_menu_image) {
                $item_output .= '<img src="' . esc_url($sub_menu_image) . '" alt="' . esc_attr($item->title) . '" class="sub-menu-image">';
            }
        }
